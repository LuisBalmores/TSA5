...
function updateTotalItems() {
    const totalItems = quantity1 + quantity2 + quantity3 + quantity4 + quantity5 + quantity6;
    document.getElementById('totalItems').innerText = totalItems;
}

function calculateTotalBill() {
    const totalBill = quantity1 * prices.item1 + quantity2 * prices.item2 + quantity3 * prices.item3 + quantity4 * prices.item4 + quantity5 * prices.item5 + quantity6 * prices.item6;
...
$('#submitOrder').click(function() {
    ...
    const invoice = `
        ...
        <p><strong>Cash Payment:</strong> $${cashInput.toFixed(2)}</p>
